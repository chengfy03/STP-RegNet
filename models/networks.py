...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    # ITTR
        ...
        net = IttrGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                            no_antialias=no_antialias, no_antialias_up=True, n_blocks=9, opt=opt)
    elif netG == 'spd':
        net = SpdGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                           no_antialias=no_antialias, no_antialias_up=True, n_blocks=6, opt=opt)
    else:
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, layers=[], encode_only=False):
        # print("ResnetGenerator:",input.shape)
        if -1 in layers:
...
class IttrGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_blocks):  # add ITTR blocks
            model += [HPB(ngf * mult, dim_head=16,
                          heads=8,
                          ff_mult=4,
    ...
    def forward(self, input, layers=[], encode_only=False):
        # print("IttrGenerator", input.shape)
        if -1 in layers:
            layers.append(len(self.model))
        if len(layers) > 0:
            feat = input
...
class ResNet(nn.Module):
    def __init__(self, block, num_block, in_channels, num_classes=100):
        super().__init__()
        ...
        #     nn.BatchNorm2d(64),
        #     nn.ReLU(inplace=True))
        self.conv1 = Focus(in_channels, 64, k=1, s=1)

        # we use a different inputsize than the original paper
        # so conv2_x's stride is 1
        self.conv2_x = self._make_layer(block, 64, num_block[0],
                                        1)  # Here in_channels = 64, and num_block[0] = 64 and s = 1
...
def resnet18(in_channels, deformation_mode):
    """
    return a ResNet 18 object
    """
    net = ResNet(BottleNeck, [3, 4, 6, 3], in_channels)

    if deformation_mode == "affine":
        fc = [nn.Linear(512 * net.expansion, 512),
              nn.Linear(512, 6)
              ]

        # 修改 resnet18 fc 层
        net.fc = nn.Sequential(*fc)
        # 初始化权重
        net.fc[1].weight.data.zero_()
        net.fc[1].bias.data.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))
        return net
    elif deformation_mode == "perspective":
        fc = [nn.Linear(512 * net.expansion, 512),
              nn.Linear(512, 8)
              ]
        net.fc = nn.Sequential(*fc)
...
class SpdGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        assert (n_blocks >= 0)
        super(SpdGenerator, self).__init__()
        self.opt = opt
        ...
        # 下采样部分
        # (64,128,128)
        conv1 = Focus(input_nc, ngf, k=1, s=1)
        # (256,128,128)
        conv2_x = self._make_layer(BottleNeck, ngf, num_block[0], 1)
        # (256,64,64)
        conv3_x = self._make_layer(BottleNeck, ngf, num_block[1], 2)
...
# # STN
# def define_stn(in_channels, net_name="resnet34",gpu_ids=[]):
#     assert (torch.cuda.is_available())
#     # theta 标准参数
#     standard_theta = torch.tensor([[[1., 0., 0.], [0., 1., 0.]]], dtype=torch.float32)
#     # theta 标准参数
#     standard_corners= torch.tensor([[[1., 0., 0.], [0., 1., 0.]]], dtype=torch.float32)
#     if len(gpu_ids) > 0:
#         standard_theta = standard_theta.to(gpu_ids[0])
#         net = AffineNet(in_channels, standard_theta, net=net_name)
#         net.to(gpu_ids[0])
#     return net
#
#
...
#         resnet34.fc = nn.Linear(in_features=512, out_features=6)
#         resnet34.fc.weight.data.zero_()
#         resnet34.fc.bias.data.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))
#         self.resnet34 = resnet34
#
#     def forward(self, x):
#         theta = self.resnet34(x)
#         return theta
#

# class AffineNet(nn.Module):
#     def __init__(self, in_channels, standard_theta, net):
#         super(AffineNet, self).__init__()
#         if net == "resnet34":
#             self.localization = Resnet34(in_channels * 2)
#         elif net == "resnet18_spd":
#             self.localization = resnet18(in_channels * 2)
#         self.standard_theta = standard_theta
#
#     def _calculate_regularization_term(self, theta):
#         x = torch.mean(torch.abs(theta - self.standard_theta))
#         return x
#
#     def stn(self, fake_B, real_A, real_B):
#         # print("Starting stn, x: ", x.size())
#         # 从真实的图像中寻找配准特征
#         xy = torch.cat([real_A, real_B], 1)
#         theta = self.localization(xy)
#         theta = theta.view(-1, 2, 3)
#         # tensor([[[1., 0., 0.],
#         #          [0., 1., 0.]]], device='cuda:0', grad_fn=<ViewBackward0>)
#         reg_term = self._calculate_regularization_term(theta)
#         # print(theta)
#         # print("theta: ", theta.size())
#         # print("X: ", x.size())
#         grid = F.affine_grid(theta, real_A.size(), False)
#         # print("Grid: ", grid.size())
#         fake_register = F.grid_sample(fake_B, grid, align_corners=False)
#         real_register = F.grid_sample(real_A, grid, align_corners=False)
#
#         return fake_register, real_register, grid, reg_term
#
#     def forward(self, fake_B, real_A, real_B):  # src, tgt):
#         # x = torch.cat([src, tgt], dim=1)
#         # transform the input
#         return self.stn(fake_B, real_A, real_B)


# 仿射变换

# STN
# deformation_mode: affine, perspective
def define_stn(in_channels, net_name="resnet34", deformation_mode="affine", gpu_ids=[]):
    assert (torch.cuda.is_available())
    # 仿射变换 theta 标准参数
    standard_theta = torch.tensor([[[1., 0., 0.], [0., 1., 0.]]], dtype=torch.float32)
    if len(gpu_ids) > 0:
        standard_theta = standard_theta.to(gpu_ids[0])
        net = AffineNet(in_channels, standard_theta, net=net_name, deformation_mode=deformation_mode)
        net.to(gpu_ids[0])
    return net


# 仿射变换
def affine_transform(net):
    net.fc = nn.Linear(in_features=512, out_features=6)
    net.fc.weight.data.zero_()
    net.fc.bias.data.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))
    return net


# 透视变换
def perspective_transform(net):
    fc = nn.Sequential(nn.Linear(512, 64),
                       nn.ReLU(),
                       nn.Linear(64, 8))
    net.fc = fc
    return net
...
class Resnet34(nn.Module):
    def __init__(self, in_channels, deformation_mode):
        super(Resnet34, self).__init__()
        resnet34 = torchvision.models.resnet34(pretrained=True)
        resnet34.conv1 = nn.Conv2d(in_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        # resnet34.fc = nn.Linear(in_features=512, out_features=6)
        ...
        # resnet34.fc.bias.data.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))
        if deformation_mode == "affine":
            resnet34 = affine_transform(resnet34)
        elif deformation_mode == "perspective":
            resnet34 = perspective_transform(resnet34)
        self.resnet34 = resnet34
...
# 配准网络 以resnet34为主干网络 realA cat realB 一起输入网络
class AffineNet(nn.Module):
    def __init__(self, in_channels, standard_theta, net, deformation_mode):
        super(AffineNet, self).__init__()
        self.standard_theta = standard_theta
        # 变换方式
        self.deformation_mode = deformation_mode
        if net == "resnet34":
            self.localization = Resnet34(in_channels * 2, deformation_mode)
        elif net == "resnet18_spd":
            self.localization = resnet18(in_channels * 2, deformation_mode)
        else:
            print("配准网络参数错误!")

    # affine smooth
    def _calculate_regularization_term_affine(self, theta):
        x = torch.mean(torch.abs(theta - self.standard_theta))
        return x

    # 仿射变换
    def stn_affine(self, fake_B, real_A, real_B):
        # print("Starting stn, x: ", x.size())
        xy = torch.cat([real_A, real_B], 1)
        theta = self.localization(xy)
        theta = theta.view(-1, 2, 3)
        # tensor([[[1., 0., 0.],
        #          [0., 1., 0.]]], device='cuda:0', grad_fn=<ViewBackward0>)
        reg_term = self._calculate_regularization_term_affine(theta)
        # print(theta)
        # print("theta: ", theta.size())
        # print("X: ", x.size())
        grid = F.affine_grid(theta, real_A.size(), False)
    ...
    # perspective smooth
    def _calculate_regularization_term_perspective(self, theta):
        x = torch.mean(torch.abs(theta))
        return x

    # 透视变换
    def stn_perspective(self, fake_B, real_A, real_B):
        # 从真实的图像中寻找配准特征
        xy = torch.cat([real_A, real_B], 1)
        theta = self.localization(xy)
        theta = theta.view(-1, 4, 2)

        reg_term = self._calculate_regularization_term_perspective(theta)
        ...
        corners_hat = corners + theta
        # 计算透视变换的矩阵 (src,dst)
        h = kornia.get_perspective_transform(corners, corners_hat)

        fake_register = kornia.warp_perspective(fake_B, h, (256, 256))
        real_register = kornia.warp_perspective(real_A, h, (256, 256))

        return fake_register, real_register, theta, reg_term

    def forward(self, fake_B, real_A, real_B):  # src, tgt):
        # x = torch.cat([src, tgt], dim=1)
        # transform the input
        if self.deformation_mode == "affine":
            return self.stn_affine(fake_B, real_A, real_B)
        elif self.deformation_mode == "perspective":
            return self.stn_perspective(fake_B, real_A, real_B)
...
class Resnet18(nn.Module):
    def __init__(self, in_channels, deformation_mode):
        ...
        resnet18.conv1 = nn.Conv2d(in_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        # if deformation_mode == "affine":
        #     resnet18 = affine_transform(resnet18)
        if deformation_mode == "perspective":
            resnet18 = perspective_transform(resnet18)
...
class Resnet34Attention(nn.Module):
    def __init__(self, in_channels, deformation_mode):
        super(Resnet34Attention, self).__init__()
        resnet34_attention = torchvision.models.resnet34(pretrained=True)
        resnet34_attention.conv1 = nn.Conv2d(in_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        # if deformation_mode == "affine":
        #     resnet18 = affine_transform(resnet18)
        if deformation_mode == "perspective":
            resnet34_attention = perspective_transform(resnet34_attention)
        self.resnet34_attention = resnet34_attention

    def forward(self, x):
        theta = self.resnet34_attention(x)
        return theta
...
# 配准网络 以resnet18为主干网络 双分支 -> 主干 分别获取 realA realB 的注意力图 att_realA cat att_realB 输入主干网络
class RegNet(nn.Module):
    def __init__(self, in_channels, net, deformation_mode):
        super(RegNet, self).__init__()
        # 通道注意力
        self.basic_block_x = BasicBlock(in_channels, 16)
        self.basic_block_y = BasicBlock(in_channels, 16)

        if net == "resnet18":
            self.localization = Resnet18(16 * 2, deformation_mode)
        elif net == "resnet34_attention":
            self.localization = Resnet34Attention(16 * 2, deformation_mode)
        else:
            print("配准网络参数错误!")
    ...
    def stn_perspective(self, fake_B, real_A, real_B):
        x_att = self.basic_block_x(real_A)
        y_att = self.basic_block_y(real_B)
        xy_att = torch.cat([x_att, y_att], 1)

        theta = self.localization(xy_att)
        theta = theta.view(-1, 4, 2)
    ...
    def forward(self, fake_B, real_A, real_B):
        return self.stn_perspective(fake_B, real_A, real_B)


def define_reg(in_channels, net_name="resnet18", deformation_mode="perspective", gpu_ids=[]):
    assert (torch.cuda.is_available())
    if len(gpu_ids) > 0:
        net = RegNet(in_channels, net=net_name, deformation_mode=deformation_mode)
        net.to(gpu_ids[0])
    return net
...
class Sobelxy(nn.Module):
    def __init__(self):
        ...
        kernely = [[1, 2, 1],
                   [0, 0, 0],
                   [-1, -2, -1]]
        kernelx = torch.FloatTensor(kernelx).unsqueeze(0).repeat(3,1,1).unsqueeze(0)
        kernely = torch.FloatTensor(kernely).unsqueeze(0).repeat(3,1,1).unsqueeze(0)
...
class GradientLoss(nn.Module):
    ...
    def forward(self, real_A, fake_B):
        real_A_grad = self.sobelconv(real_A)
        fake_B_grad = self.sobelconv(fake_B)
        loss_grad = F.l1_loss(real_A_grad, fake_B_grad)
        return loss_grad

# SPD 测试
# net = SpdGenerator(3, 3, n_blocks=6)
# x = torch.empty((1, 3, 256, 256)).normal_()
# # net(x, [0, 4, 8, 12, 16], encode_only=True)
# net(x)

# ResNet18 测试
# net = AffineNet(in_channels=3)
# x = torch.empty((1, 3, 256, 256)).normal_()
# y = torch.empty((1, 3, 256, 256)).normal_()
# net(x,y)

#
# net = RegNet(in_channels=3, net='resnet18', deformation_mode='perspective')
# print(net)
# summary(net, [(1, 3, 256, 256), (1, 3, 256, 256), (1, 3, 256, 256)], device="cuda:0")
